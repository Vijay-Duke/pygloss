...
internal object HttpRunner {
    ...
    fun executeWithRetry(
        ...
        timeout: Duration,
        maxAttempts: Int,
        shouldRetryResponse: (Int) -> Boolean = { false },
    ): HttpOutcome {
        val attempts = maxAttempts.coerceAtLeast(1)
        repeat(attempts) { attempt ->
            when (val outcome = executeOnce(httpClient, request, timeout)) {
                is HttpOutcome.Response -> {
                    if (!shouldRetryResponse(outcome.response.statusCode) || attempt == attempts - 1) {
                        return outcome
                    }
                    sleepBeforeRetry() ?: return HttpOutcome.Network
                }
    ...
    private fun sleepBeforeRetry(): Unit? {
        return try {
    ...
    private fun executeOnce(httpClient: HttpClient, request: HttpRequest, timeout: Duration): HttpOutcome {
        ...
        return try {
            ...
            when (e.cause) {
                is SocketTimeoutException, is HttpTimeoutException -> HttpOutcome.Timeout
