...
class ModelCacheService(private val project: Project) : Disposable {
    ...
    fun writeLlmBody(blockHash: String, body: String) {
        synchronized(lock) {
            diskStore.write(blockHash, body)
            llmBodyKeysByBlockHash[blockHash] = blockHash
        }
    }
    ...
    fun readLlmBody(blockHash: String): String? {
        synchronized(lock) {
            return llmBodyKeysByBlockHash[blockHash]?.let(diskStore::read)
        }
    }
    ...
    private fun removeLlmBodyMappingLocked(blockHash: String) {
        llmBodyKeysByBlockHash.remove(blockHash)
    }
